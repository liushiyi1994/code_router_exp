...
This M3 dry-run collects generic cheap-probe observations from local model outputs. It is not evidence that probes close the observability gap.
...
- `knn_label_entropy` and `knn_winner_entropy` are null in this dry-run because no aligned train-only state embeddings were supplied.
