...
Cost is a first-class part of the method, not an afterthought. For local/open models, record token counts, latency, and a GPU-runtime proxy. For closed-source provider models, record input/output tokens, model version/date, request count, and provider price snapshot. Provider pricing must be refreshed before a cost-aware run and documented with source URLs and checked dates.

Closed-source provider families reserved for later provider-aware experiments:
...
These models should be included in the model-pool and calibration plan once API access and budget are explicitly enabled. They are not part of the default local vLLM pipeline.
...
### 9.2.1 Later closed-source provider pool

After benchmark and local vLLM experiments support the method, add a small explicit-budget provider-aware pool. The first provider pool should include representative models from:
...
Use these models first for calibration and held-out/new-model experiments, not full query--model matrices. Every run must save provider, model version, date checked, token counts, latency, request count, exact pricing source URLs, and cost-adjusted utility.
